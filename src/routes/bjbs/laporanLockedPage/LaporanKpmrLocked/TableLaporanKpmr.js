...
class TableLaporanKpmr extends  React.Component{
    ...
    componentWillReceiveProps(nextProps) {
      this.setState({
        lockedreportdata: nextProps.lockedreportdata.data
      })
    }
    ...
    render() {
        ...
        return (
            <Card title={`Data Laporan Risiko Inheren Locked - ${this.state.risk_name} : ${monthText} ${this.state.paramtahun}`}>
                {
                  addbutton ? <h1>...To be advised</h1> :
                    <>
                            ...
                            <Spin tip="Loading..." spinning={loading}>
                                <Table dataSource={this.state.lockedreportdata} className="gx-table-responsive" onChange={this.handleChange} rowKey="id" columns={columns}
                                       pagination={false}
                                       size="small"
                                       />
...
const mapStateToProps = ({
  ...
  lockedreport
    ...
    const {lockedreportdata} = lockedreport;
