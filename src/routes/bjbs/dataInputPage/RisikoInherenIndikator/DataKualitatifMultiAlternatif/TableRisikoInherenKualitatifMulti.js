...
class TableRisikoInherenKualitatifMulti extends  React.Component{
    constructor(props) {
        super(props);
        this.state = {
            sortedInfo: null,
            ...
            edname:'',
            paging:1,
            paramrisk_id : props.fetchdata ? props.fetchdata[0].risk_id : 0,
            paramrisk : props.fetchdata ? props.fetchdata[0].risks : 0,
            ...
            toBeSaved: {
              ingredients_id: 0,
              ratio_indikators_id: 0,
              parameter_version_id: props.fetchdata ? props.fetchdata[0].version_id : 0,
              value: 0,
              bulan: props.fetchdata ? props.fetchdata[0].ismonth : 0,
              tahun: props.fetchdata ? props.fetchdata[0].isyear : 0
            },
            ingredients_id: 0
        }
    }
    ...
    render() {
        ...
        const columns = [
        {
            ...
            render: risk_rate => (
              <>
                {
                  <Tag
                    color={risk_rate === 'H' ? 'red' : risk_rate === "MTH" ? 'orange' : risk_rate === "M" ? "yellow" : risk_rate === "LTM" ? 'green' : risk_rate === "L" ? "blue" : "black" }
                    key={risk_rate}
                  >
                    {risk_rate}
                  </Tag>
        ...
        return (
            <Card title={`Data Risiko Inheren Kualitatif Multil Alternatif - ${this.state.risk_name} : ${monthText} ${this.state.paramtahun} (${this.state.version_name})`}>
                {
...
const mapStateToProps = ({
  ...
  risikoinhereninputkualitatif
    ...
    const {risikoinhereninputkualitatifdata, addrisikoinhereninputkualitatifresult} = risikoinhereninputkualitatif;
    return {
      token,
      getallparametermanualtable,
      postparametermanual,
...
export default connect(mapStateToProps, {
  ...
  addRisikoInherenInputKualitatif,
  resetAddRisikoInherenInputKualitatif
})(TableRisikoInherenKualitatifMulti);
